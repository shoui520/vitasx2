	...
	u32 block_cycles, u32 packed_guests)
{
	const VitaPerformanceTelemetry::ScopedCpuStage profile_stage(
		VitaPerformanceTelemetry::CpuStage::EeHelper);
	// PCSX2 owners: R5900OpcodeImpl.cpp::ADDIU()/SLTIU()/SQ(),
	// Interpreter.cpp::BEQ(), x86 recVTLB.cpp::DynGen_DirectWrite(), and
	// iR5900.cpp::iBranchTest(). The compiler has proved two invariant-value
	// SQs, a pointer stride of 32, and a count-down-by-32 self edge.
	const unsigned pointer_guest = packed_guests & 0x1f;
	const unsigned count_guest = (packed_guests >> 5) & 0x1f;
	const unsigned value_guest = (packed_guests >> 10) & 0x1f;
	const unsigned result_guest = (packed_guests >> 15) & 0x1f;
	const u32 address = cpuRegs.GPR.r[pointer_guest].UL[0];
	const u32 count_low = cpuRegs.GPR.r[count_guest].UL[0];
	const bool zero_value = value_guest == 0 ||
		(cpuRegs.GPR.r[value_guest].UD[0] |
			cpuRegs.GPR.r[value_guest].UD[1]) == 0;
	const r128 value = zero_value ? r128_zero() :
		r128_load(&cpuRegs.GPR.r[value_guest]);
	const u32 aligned_address = address & ~0x0fu;
	u32 iterations = 1;
	bool state_published = false;
	bool force_redispatch = false;
	u32 bulk_bytes = 0;
	bool scalar_iteration = false;

#if defined(VITASX2_QEMU_VALIDATION)
	bool direct_bulk = false;
	extern u32 g_qemuPairQwordFillHelperCalls;
	extern u32 g_qemuPairQwordFillBulkChunks;
	extern u32 g_qemuPairQwordFillBulkQwords;
	extern u32 g_qemuPairQwordFillScalarIterations;
	extern u32 g_qemuPairQwordFillPageReturns;
	extern u32 g_qemuPairQwordFillRedispatches;
	extern bool g_qemuPairQwordFillForceRedispatch;
	g_qemuPairQwordFillHelperCalls++;
#endif

	const VTLBVirtual mapping =
		vtlbdata.vmap[aligned_address >> VTLB_PAGE_BITS];
	u32 physical_address = 0;
	bool protected_page = false;
	if (!mapping.isHandler(aligned_address))
	{
		protected_page = VitaEeGetDirectRamProtection(
			mapping, aligned_address, &physical_address);
#if defined(VITASX2_QEMU_VALIDATION)
		protected_page |= g_qemuPairQwordFillForceRedispatch;
#endif
	}

	const u32 bytes_to_page =
		VTLB_PAGE_SIZE - (aligned_address & VTLB_PAGE_MASK);
	const bool batchable = !mapping.isHandler(aligned_address) &&
		!protected_page && bytes_to_page >= 2 * sizeof(u128) &&
		block_cycles != 0;
	if (batchable)
	{
		iterations = bytes_to_page / (2 * sizeof(u128));

		// ADDIU count,count,-32 writes sign_extend_32(). SLTIU becomes true
		// only when that result is in [0,31]. Subtracting 32 preserves the
		// low five bits, so the first such result is count_low >> 5 iterations
		// away, or one complete 32-bit orbit for an initial value below 32.
		const u32 to_completion = (count_low >> 5) != 0 ?
			(count_low >> 5) : (1u << 27);
		iterations = std::min(iterations, to_completion);
		...
		else
		{
			const u64 distance = static_cast<u64>(
				-static_cast<s64>(cycle_delta));
			const u64 chunk_cycles =
				static_cast<u64>(iterations) * block_cycles;
			if (distance <= chunk_cycles)
			{
				to_event = static_cast<u32>(
					(distance + block_cycles - 1) / block_cycles);
			}
		}
		iterations = std::min(iterations, to_event);

		u8* const host = reinterpret_cast<u8*>(
			mapping.assumePtr(aligned_address));
		const u32 qword_count = iterations * 2;
		bulk_bytes = qword_count * sizeof(u128);
		if (zero_value)
		{
			std::memset(host, 0, bulk_bytes);
		}
		else
		{
			for (u32 iteration = 0; iteration < iterations; iteration++)
			{
				u8* const pair = host + iteration * 2 * sizeof(u128);
				r128_store(pair, value);
				r128_store(pair + sizeof(u128), value);
			}
		}
		force_redispatch |= NotifyVitaEeRamWrite(host, bulk_bytes) != 0;
#if defined(VITASX2_QEMU_VALIDATION)
		direct_bulk = true;
		g_qemuPairQwordFillBulkChunks++;
		g_qemuPairQwordFillBulkQwords += qword_count;
#endif
	}
	else
	{
		// Preserve the architectural order within one iteration: first SQ;
		// count ADDIU; pointer ADDIU; SLTIU; second SQ; branch; delay ADDIU.
		vtlb_memWrite128(aligned_address, value);
		if (protected_page)
		{
			force_redispatch = true;
			if (Cpu && Cpu->Clear)
				Cpu->Clear(physical_address & ~3u, 4);
		}

		const u32 updated_count_low = count_low - 32;
		cpuRegs.GPR.r[count_guest].UD[0] = static_cast<u64>(
			static_cast<s64>(static_cast<s32>(updated_count_low)));
		const u32 first_pointer_low = address + 16;
		cpuRegs.GPR.r[pointer_guest].UD[0] = static_cast<u64>(
			static_cast<s64>(static_cast<s32>(first_pointer_low)));
		const u64 predicate = cpuRegs.GPR.r[count_guest].UD[0] < 32 ? 1 : 0;
		cpuRegs.GPR.r[result_guest].UD[0] = predicate;

		const u32 second_address = first_pointer_low & ~0x0fu;
		const VTLBVirtual second_mapping =
			vtlbdata.vmap[second_address >> VTLB_PAGE_BITS];
		u32 second_physical_address = 0;
		bool second_protected = false;
		if (!second_mapping.isHandler(second_address))
		{
			second_protected = VitaEeGetDirectRamProtection(
				second_mapping, second_address, &second_physical_address);
#if defined(VITASX2_QEMU_VALIDATION)
			second_protected |= g_qemuPairQwordFillForceRedispatch;
#endif
		}
		vtlb_memWrite128(second_address, value);
		if (second_protected)
		{
			force_redispatch = true;
			if (Cpu && Cpu->Clear)
				Cpu->Clear(second_physical_address & ~3u, 4);
		}

		cpuRegs.GPR.r[pointer_guest].UD[0] = static_cast<u64>(
			static_cast<s64>(static_cast<s32>(address + 32)));
		state_published = true;
		scalar_iteration = true;
#if defined(VITASX2_QEMU_VALIDATION)
		g_qemuPairQwordFillScalarIterations++;
#endif
	}

	if (!state_published)
	{
		const u32 updated_count_low = count_low - iterations * 32;
		cpuRegs.GPR.r[count_guest].UD[0] = static_cast<u64>(
			static_cast<s64>(static_cast<s32>(updated_count_low)));
		cpuRegs.GPR.r[pointer_guest].UD[0] = static_cast<u64>(
			static_cast<s64>(static_cast<s32>(address + iterations * 32)));
		cpuRegs.GPR.r[result_guest].UD[0] =
			cpuRegs.GPR.r[count_guest].UD[0] < 32 ? 1 : 0;
	}

	cpuRegs.cycle += static_cast<u64>(iterations) * block_cycles;
	const bool repeat = cpuRegs.GPR.r[result_guest].UD[0] == 0;
	cpuRegs.pc = repeat ? start_pc : fallthrough_pc;
	const bool event_due = static_cast<s32>(static_cast<u32>(cpuRegs.cycle) -
		static_cast<u32>(cpuRegs.nextEventCycle)) >= 0;
	const bool page_return = repeat && !event_due && !force_redispatch &&
		bulk_bytes != 0;
	VitaPerformanceTelemetry::RecordEePairQwordFillExecutionIfProfiling(
		bulk_bytes, zero_value && bulk_bytes != 0, scalar_iteration,
		page_return, force_redispatch);
	if (event_due)
		return VITA_EE_PAIR_QWORD_FILL_EVENT;
	if (force_redispatch)
	{
#if defined(VITASX2_QEMU_VALIDATION)
		g_qemuPairQwordFillRedispatches++;
#endif
		return VITA_EE_PAIR_QWORD_FILL_REDISPATCH;
	}
	if (!repeat)
		return VITA_EE_PAIR_QWORD_FILL_COMPLETE;
#if defined(VITASX2_QEMU_VALIDATION)
	if (direct_bulk)
		g_qemuPairQwordFillPageReturns++;
#endif
	return VITA_EE_PAIR_QWORD_FILL_SELF;
}
	...
	u32 packed_cycles, u32 packed_guests)
{
	...
	const unsigned end_guest = (packed_guests >> 15) & 0x1f;
	const u32 block_cycles = packed_cycles & 0x3ff;
	const u32 load_cycles = (packed_cycles >> 10) & 0x3ff;
	const u32 store_prefix_cycles = (packed_cycles >> 20) & 0x3ff;
	...
	const u64 end = cpuRegs.GPR.r[end_guest].UD[0];
	u32 iterations = 1;
	bool pointers_published = false;
	bool force_redispatch = false;

#if defined(VITASX2_QEMU_VALIDATION)
	bool direct_bulk = false;
	extern u32 g_qemuWordCopyHelperCalls;
	extern u32 g_qemuWordCopyBulkChunks;
	extern u32 g_qemuWordCopyBulkWords;
	extern u32 g_qemuWordCopyScalarIterations;
	extern u32 g_qemuWordCopyPageReturns;
	extern u32 g_qemuWordCopyRedispatches;
	extern bool g_qemuWordCopyForceRedispatch;
	g_qemuWordCopyHelperCalls++;
#endif

	if ((source_address & 3u) != 0)
	{
		cpuRegs.pc = start_pc + sizeof(u32);
		cpuRegs.cycle += load_cycles;
		if (Cpu && Cpu->CancelInstruction)
			Cpu->CancelInstruction();
		return VITA_EE_WORD_COPY_EVENT;
	}
	...
	const bool batchable = !source_mapping.isHandler(source_address) &&
		...
		!destination_protected && (destination_address & 3u) == 0 &&
		(source_address & 0xffffe000u) != 0x10000000u && block_cycles != 0;
	if (batchable)
	{
		iterations = std::min(source_bytes_to_page, destination_bytes_to_page) / 4;
		...
		if (canonical_end)
		{
			const u32 bytes_to_end = end_low - source_address;
			if (bytes_to_end != 0 && (bytes_to_end & 3u) == 0)
				iterations = std::min(iterations, bytes_to_end / 4);
		}
		...
		const u32* const source_host =
			reinterpret_cast<const u32*>(source_mapping.assumePtr(source_address));
		u32* const destination_host =
			reinterpret_cast<u32*>(destination_mapping.assumePtr(destination_address));
		u32 last_value = 0;
		// Do not use memcpy/memmove: the EE loop is a forward load-then-store
		// recurrence, so an overlapping destination ahead of the source can alter
		// values loaded by later iterations.
		for (u32 i = 0; i < iterations; i++)
		{
...
#if defined(VITASX2_QEMU_VALIDATION)
		direct_bulk = true;
		g_qemuWordCopyBulkChunks++;
		g_qemuWordCopyBulkWords += iterations;
#endif
	}
	else
	{
		const u32 value = vtlb_memRead<mem32_t>(source_address);
		cpuRegs.GPR.r[value_guest].UD[0] = static_cast<u64>(
			static_cast<s64>(static_cast<s32>(value)));
		if ((source_address & 0xffffe000u) == 0x10000000u)
		{
			// EmitCounterReadEventExit() always publishes the post-LW PC/cycles
			// and enters the event dispatcher before the following ADDIU.
			cpuRegs.pc = start_pc + sizeof(u32);
			cpuRegs.cycle += load_cycles;
			return VITA_EE_WORD_COPY_EVENT;
		}
		...
		cpuRegs.GPR.r[source_guest].UD[0] = static_cast<u64>(
			static_cast<s64>(static_cast<s32>(source_address + 4)));
		if ((destination_address & 3u) != 0)
		{
			cpuRegs.pc = start_pc + 3 * sizeof(u32);
			cpuRegs.cycle += store_prefix_cycles;
			if (Cpu && Cpu->CancelInstruction)
				Cpu->CancelInstruction();
			return VITA_EE_WORD_COPY_EVENT;
		}

		const VTLBVirtual scalar_destination_mapping =
			vtlbdata.vmap[destination_address >> VTLB_PAGE_BITS];
		u32 destination_physical = 0;
		...
		vtlb_memWrite<mem32_t>(destination_address, value);
		if (scalar_destination_protected)
		{
			force_redispatch = true;
			if (Cpu && Cpu->Clear)
		...
		cpuRegs.GPR.r[destination_guest].UD[0] = static_cast<u64>(
			static_cast<s64>(static_cast<s32>(destination_address + 4)));
		pointers_published = true;
#if defined(VITASX2_QEMU_VALIDATION)
		g_qemuWordCopyScalarIterations++;
#endif
	}

	if (!pointers_published)
	{
		cpuRegs.GPR.r[source_guest].UD[0] = static_cast<u64>(
			static_cast<s64>(static_cast<s32>(source_address + iterations * 4)));
		cpuRegs.GPR.r[destination_guest].UD[0] = static_cast<u64>(
			static_cast<s64>(static_cast<s32>(destination_address + iterations * 4)));
	}
	cpuRegs.cycle += static_cast<u64>(iterations) * block_cycles;
	const bool repeat = cpuRegs.GPR.r[source_guest].UD[0] != end;
	cpuRegs.pc = repeat ? start_pc : fallthrough_pc;
	const bool event_due = static_cast<s32>(static_cast<u32>(cpuRegs.cycle) -
		static_cast<u32>(cpuRegs.nextEventCycle)) >= 0;
	if (event_due)
	...
	{
#if defined(VITASX2_QEMU_VALIDATION)
		g_qemuWordCopyRedispatches++;
#endif
		return VITA_EE_WORD_COPY_REDISPATCH;
	}
	if (!repeat)
		return VITA_EE_WORD_COPY_COMPLETE;
#if defined(VITASX2_QEMU_VALIDATION)
	if (direct_bulk)
		g_qemuWordCopyPageReturns++;
#endif
