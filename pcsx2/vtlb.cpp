...
static void VitaEeClampSignedHalfwordsAndPack(const GPR_reg& clamp,
	GPR_reg (&values)[8], u8* destination)
{
	// PCSX2 owners: MMI.cpp::PMINH(), PMAXH(), and PPACB(). The clamp is
	// signed in each halfword. PPACB then selects the low byte of every
	// halfword from Rt followed by Rs.
#if defined(__ARM_NEON) || defined(__ARM_NEON__)
	const int16x8_t clamp_vector = vld1q_s16(clamp.SS);
	const int16x8_t zero = vdupq_n_s16(0);
	int16x8_t vectors[8];
	for (unsigned i = 0; i < 8; i++)
		vectors[i] = vld1q_s16(values[i].SS);
	for (unsigned pair = 0; pair < 4; pair++)
	{
		const unsigned even = pair * 2;
		const unsigned odd = even + 1;
		vectors[even] = vmaxq_s16(vminq_s16(vectors[even], clamp_vector), zero);
		vectors[odd] = vmaxq_s16(vminq_s16(vectors[odd], clamp_vector), zero);
		const uint8x16_t packed = vcombine_u8(
			vmovn_u16(vreinterpretq_u16_s16(vectors[even])),
			vmovn_u16(vreinterpretq_u16_s16(vectors[odd])));
		vst1q_s16(values[even].SS, vectors[even]);
		vst1q_u8(values[odd].UC, packed);
		if (destination)
			vst1q_u8(destination + pair * 16, packed);
	}
#else
	for (unsigned pair = 0; pair < 4; pair++)
	{
		...
		{
			const s16 upper = clamp.SS[lane];
			const s16 even_min = values[even].SS[lane] < upper ?
				values[even].SS[lane] : upper;
			const s16 odd_min = values[odd].SS[lane] < upper ?
				values[odd].SS[lane] : upper;
			values[even].SS[lane] = even_min > 0 ? even_min : 0;
			values[odd].SS[lane] = odd_min > 0 ? odd_min : 0;
		}
		GPR_reg packed{};
		for (unsigned lane = 0; lane < 8; lane++)
		{
			packed.UC[lane] = values[even].UC[lane * 2];
			packed.UC[lane + 8] = values[odd].UC[lane * 2];
		}
		values[odd] = packed;
		if (destination)
			std::memcpy(destination + pair * 16, packed.UC, 16);
	}
#endif
}
	...
	GPR_reg* final_values, GPR_reg* final_temporary)
{
	...
	const int16x8_t zero = vdupq_n_s16(0);
	int16x8_t v0 = vld1q_s16(reinterpret_cast<const s16*>(source + 0));
	int16x8_t v1 = vld1q_s16(reinterpret_cast<const s16*>(source + 16));
	int16x8_t v2 = vld1q_s16(reinterpret_cast<const s16*>(source + 32));
	int16x8_t v3 = vld1q_s16(reinterpret_cast<const s16*>(source + 48));
	int16x8_t v4 = vld1q_s16(reinterpret_cast<const s16*>(source + 64));
	...
	v3 = vmaxq_s16(vminq_s16(v3, upper), zero);
	v4 = vmaxq_s16(vminq_s16(v4, upper), zero);
	v5 = vmaxq_s16(vminq_s16(v5, upper), zero);
	v6 = vmaxq_s16(vminq_s16(v6, upper), zero);
	v7 = vmaxq_s16(vminq_s16(v7, upper), zero);
	const uint8x16_t p01 = vcombine_u8(
		vmovn_u16(vreinterpretq_u16_s16(v0)),
		vmovn_u16(vreinterpretq_u16_s16(v1)));
	const uint8x16_t p23 = vcombine_u8(
		vmovn_u16(vreinterpretq_u16_s16(v2)),
		vmovn_u16(vreinterpretq_u16_s16(v3)));
	const uint8x16_t p45 = vcombine_u8(
		vmovn_u16(vreinterpretq_u16_s16(v4)),
		vmovn_u16(vreinterpretq_u16_s16(v5)));
	const uint8x16_t p67 = vcombine_u8(
		vmovn_u16(vreinterpretq_u16_s16(v6)),
		vmovn_u16(vreinterpretq_u16_s16(v7)));
	vst1q_u8(destination + 0, p01);
	vst1q_u8(destination + 16, p23);
	vst1q_u8(destination + 32, p45);
	vst1q_u8(destination + 48, p67);
	if (final_values)
	{
		vst1q_s16(final_values[0].SS, v0);
		vst1q_u8(final_values[1].UC, p01);
		vst1q_s16(final_values[2].SS, v2);
		vst1q_u8(final_values[3].UC, p23);
		vst1q_s16(final_values[4].SS, v4);
		vst1q_u8(final_values[5].UC, p45);
		vst1q_s16(final_values[6].SS, v6);
		vst1q_u8(final_values[7].UC, p67);
	}
#else
	GPR_reg values[8]{};
	...
	u32 packed_guests0, u32 packed_guests1)
{
	...
	unsigned data_guests[8] = {
		...
		(packed_guests0 >> 20) & 0x1f,
		(packed_guests0 >> 25) & 0x1f,
		packed_guests1 & 0x1f,
		(packed_guests1 >> 5) & 0x1f,
		(packed_guests1 >> 10) & 0x1f,
	...
	const GPR_reg clamp = cpuRegs.GPR.r[clamp_guest];
	GPR_reg final_values[8]{};
	u32 iterations = 1;
	bool force_redispatch = false;

#if defined(VITASX2_QEMU_VALIDATION)
	bool direct_bulk = false;
	extern u32 g_qemuSignedHalfwordBytePackHelperCalls;
	extern u32 g_qemuSignedHalfwordBytePackBulkChunks;
	extern u32 g_qemuSignedHalfwordBytePackBulkIterations;
	extern u32 g_qemuSignedHalfwordBytePackScalarIterations;
	extern u32 g_qemuSignedHalfwordBytePackPageReturns;
	extern u32 g_qemuSignedHalfwordBytePackRedispatches;
	extern bool g_qemuSignedHalfwordBytePackForceRedispatch;
	g_qemuSignedHalfwordBytePackHelperCalls++;
#endif
	...
	u32 destination_physical = 0;
	bool destination_protected = false;
	if (!destination_mapping.isHandler(aligned_destination))
	{
		destination_protected = VitaEeGetDirectRamProtection(
			destination_mapping, aligned_destination, &destination_physical);
#if defined(VITASX2_QEMU_VALIDATION)
		destination_protected |= g_qemuSignedHalfwordBytePackForceRedispatch;
#endif
	}
	const u32 source_bytes_to_page =
...
#if defined(VITASX2_QEMU_VALIDATION)
		direct_bulk = true;
		g_qemuSignedHalfwordBytePackBulkChunks++;
		g_qemuSignedHalfwordBytePackBulkIterations += iterations;
#endif
	}
	else
	{
	...
	if (batchable)
	{
		const u32 final_counter = initial_counter - iterations * 4;
		cpuRegs.GPR.r[counter_guest].UD[0] = static_cast<u64>(
			static_cast<s64>(static_cast<s32>(final_counter)));
	...
	const bool event_due = static_cast<s32>(
		...
		static_cast<u32>(cpuRegs.nextEventCycle)) >= 0;
	if (event_due)
		return VITA_EE_SIGNED_HALFWORD_BYTE_PACK_EVENT;
	if (force_redispatch)
	{
#if defined(VITASX2_QEMU_VALIDATION)
		g_qemuSignedHalfwordBytePackRedispatches++;
#endif
		return VITA_EE_SIGNED_HALFWORD_BYTE_PACK_REDISPATCH;
	}
	if (!repeat)
		return VITA_EE_SIGNED_HALFWORD_BYTE_PACK_COMPLETE;
#if defined(VITASX2_QEMU_VALIDATION)
	if (direct_bulk)
		g_qemuSignedHalfwordBytePackPageReturns++;
#endif
	return VITA_EE_SIGNED_HALFWORD_BYTE_PACK_SELF;
}
