...
namespace VitaA32
{
	namespace
	{
		...
		constexpr u32 LDRSB_IMM_POST_INDEX = 0x00d000d0u;
		...
		constexpr u32 VDUP_I32_D_CORE = 0xee800b10u;
		constexpr u32 VDUP_I32_D_LANE = 0xf3b40c00u;
		constexpr u32 VDUP_I32_Q_CORE = 0xeea00b10u;
		constexpr u32 VDUP_I32_Q_LANE = 0xf3b40c40u;
		// ARM ARM A8.6.336, VMOV.I32 Qd,#imm8 shifted by a whole-byte lane.
		constexpr u32 VMOV_I32_Q_IMM = 0xf2800050u;
		...
		constexpr u32 VCVT_F32_S32_D = 0xf3bb0600u;
		constexpr u32 VCVT_S32_F32_D = 0xf3bb0700u;
		constexpr u32 VCVT_F32_S32_Q = 0xf3bb0640u;
		constexpr u32 VCVT_S32_F32_Q = 0xf3bb0740u;
		// ARM ARM A8.6.296. The fixed-point form encodes 64-fractional_bits
		// in imm6 and folds the power-of-two scale into the conversion.
		constexpr u32 VCVT_F32_S32_FIXED_D = 0xf2800e10u;
		constexpr u32 VCVT_S32_F32_FIXED_D = 0xf2800f10u;
		constexpr u32 VCVT_F32_S32_FIXED_Q = 0xf2800e50u;
		constexpr u32 VCVT_S32_F32_FIXED_Q = 0xf2800f50u;
		...
		constexpr u32 VADD_F32_D = 0xf2000d00u;
		constexpr u32 VSUB_F32_D = 0xf2200d00u;
		constexpr u32 VMUL_F32_D = 0xf3000d10u;
		constexpr u32 VMLA_F32_D = 0xf2000d10u;
		constexpr u32 VMLS_F32_D = 0xf2200d10u;
		// ARM ARM A8.6.324/A8.6.328 scalar-by-element forms. The scalar
		// source is one of two 32-bit lanes in D0-D15; bit 5 selects lane 1.
		constexpr u32 VMUL_F32_D_LANE = 0xf2a00940u;
		constexpr u32 VMLA_F32_D_LANE = 0xf2a00140u;
		constexpr u32 VMLS_F32_D_LANE = 0xf2a00540u;
		constexpr u32 VADD_F32_Q = 0xf2000d40u;
		constexpr u32 VSUB_F32_Q = 0xf2200d40u;
		constexpr u32 VMUL_F32_Q = 0xf3000d50u;
		constexpr u32 VMLA_F32_Q = 0xf2000d50u;
		constexpr u32 VMLS_F32_Q = 0xf2200d50u;
		constexpr u32 VMUL_F32_Q_LANE = 0xf3a00940u;
		constexpr u32 VMLA_F32_Q_LANE = 0xf3a00140u;
		constexpr u32 VMLS_F32_Q_LANE = 0xf3a00540u;
		// ARM ARM A8.6.371/A8.6.372 and A8.6.378/A8.6.379. Cortex-A9
		// implements the estimate/refinement operations in the Advanced SIMD
		// pipeline; D forms process two F32 lanes in one MPE issue cycle.
		constexpr u32 VRECPE_F32_D = 0xf3bb0500u;
		constexpr u32 VRECPS_F32_D = 0xf2000f10u;
		constexpr u32 VRSQRTE_F32_D = 0xf3bb0580u;
		constexpr u32 VRSQRTS_F32_D = 0xf2200f10u;
		...
		// ARM ARM A8.6.411, VSRI.32 Qd,Qm,#imm.
		constexpr u32 VSRI_I32_Q = 0xf3800450u;
		...
		constexpr u32 VCEQ_I32_D = 0xf3200810u;
		constexpr u32 VCEQ_I32_Q = 0xf3200850u;
		// ARM ARM A8.6.281/A8.6.290 Advanced SIMD compare-to-zero forms.
		constexpr u32 VCEQ_I32_ZERO_D = 0xf3b90100u;
		constexpr u32 VCEQ_I32_ZERO_Q = 0xf3b90140u;
		constexpr u32 VCLT_S32_ZERO_Q = 0xf3b90240u;
		...
		constexpr u32 VEXT_I8_D = 0xf2b00000u;
		constexpr u32 VEXT_I8_Q = 0xf2b00040u;
		constexpr u32 VAND_D = 0xf2000110u;
		constexpr u32 VAND_Q = 0xf2000150u;
		// ARM ARM A8.6.279, VBIT Qd,Qn,Qm (op=0b10, Q=1).
		constexpr u32 VBIT_D = 0xf3200110u;
		constexpr u32 VBIT_Q = 0xf3200150u;
		...
		constexpr u32 STMIA = 0x08800000u;
		constexpr u32 LDMIA = 0x08900000u;
	...
	CodeBuffer::CodeBuffer(CodeBuffer&& other) noexcept
		...
		, m_write_base(std::exchange(other.m_write_base, nullptr))
		, m_capacity(std::exchange(other.m_capacity, 0))
		, m_write_capacity(std::exchange(other.m_write_capacity, 0))
	...
	bool CodeBuffer::EmitU32(u32 instruction)
	{
		...
		if (!EnsureWritable(m_offset + sizeof(instruction)))
			return false;

		const size_t instruction_offset = m_offset;
		std::memcpy(m_write_base + instruction_offset, &instruction, sizeof(instruction));
		...
		Condition condition)
	{
		if (!IsRegister(rd) || !IsRegister(rn))
			return false;
		if (!set_flags && condition == Condition::AL && value == 0 &&
			CanElideSameRegisterWrite(rd, rn))
			return true;
		return EmitU32(EncodeAddImm8(rd, rn, value, set_flags, condition));
	}
	...
	bool CodeBuffer::EmitBfi(unsigned rd, unsigned rn, u8 lsb, u8 width)
	{
		if (!IsLowRegister(rd) || !IsLowRegister(rn) || lsb >= 32 || width == 0 || width > 32 - lsb)
			return false;
		return EmitU32(EncodeBfi(rd, rn, lsb, width));
	}
		...
		Condition condition)
	{
		if (!IsLowRegister(rd) || !IsLowRegister(rn) || offset > 0x0fff)
			return false;
		return EmitU32(EncodeStrImm12(rd, rn, offset, condition));
	}
	...
	bool CodeBuffer::EmitLdrsbImm8PostIndex(unsigned rd, unsigned rn, u8 offset)
	{
		if (!IsLowRegister(rd) || !IsLowRegister(rn) || rd == rn)
			return false;
		return EmitU32(EncodeLdrsbImm8PostIndex(rd, rn, offset));
	}
	...
	bool CodeBuffer::EmitVdupI32DFromCore(unsigned dd, unsigned rt)
	{
		if (!IsDRegister(dd) || !IsGeneralRegister(rt))
			return false;
		return EmitU32(EncodeVdupI32DFromCore(MapNeonDRegister(dd), rt));
	}

	bool CodeBuffer::EmitVdupI32DFromQlane(unsigned dd, unsigned qm, u8 lane)
	{
		if (!IsDRegister(dd) || !IsQRegister(qm) || lane > 3)
			return false;
		return EmitU32(EncodeVdupI32DFromQlane(MapNeonDRegister(dd),
			MapNeonQRegister(qm), lane));
	}
	...
	bool CodeBuffer::EmitVdupI32QFromQlane(unsigned qd, unsigned qm, u8 lane)
	{
		if (!IsQRegister(qd) || !IsQRegister(qm) || lane > 3)
			return false;
		return EmitU32(EncodeVdupI32QFromQlane(
			MapNeonQRegister(qd), MapNeonQRegister(qm), lane));
	}

	bool CodeBuffer::EmitVmovI32Q(unsigned qd, u8 imm8, u8 left_shift)
	{
		if (!IsQRegister(qd) || left_shift > 24 || (left_shift & 7u) != 0)
			return false;
		return EmitU32(EncodeVmovI32Q(MapNeonQRegister(qd), imm8, left_shift));
	}

	bool CodeBuffer::EmitVbicI32Q(unsigned qd, u8 imm8, u8 left_shift)
	{
		if (!IsQRegister(qd) || left_shift > 24 || (left_shift & 7u) != 0)
			return false;
		return EmitU32(EncodeVbicI32Q(MapNeonQRegister(qd), imm8, left_shift));
	}
	...
	bool CodeBuffer::EmitVcvtF32S32Q(unsigned qd, unsigned qm, unsigned fractional_bits)
	{
		if (!IsQRegister(qd) || !IsQRegister(qm) || fractional_bits > 32)
			return false;
		return EmitU32(EncodeVcvtF32S32Q(MapNeonQRegister(qd),
			MapNeonQRegister(qm), fractional_bits));
	}

	bool CodeBuffer::EmitVcvtF32S32D(unsigned dd, unsigned dm, unsigned fractional_bits)
	{
		if (!IsDRegister(dd) || !IsDRegister(dm) || fractional_bits > 32)
			return false;
		return EmitU32(EncodeVcvtF32S32D(MapNeonDRegister(dd),
			MapNeonDRegister(dm), fractional_bits));
	}

	bool CodeBuffer::EmitVcvtS32F32D(unsigned dd, unsigned dm, unsigned fractional_bits)
	{
		if (!IsDRegister(dd) || !IsDRegister(dm) || fractional_bits > 32)
			return false;
		return EmitU32(EncodeVcvtS32F32D(MapNeonDRegister(dd),
			MapNeonDRegister(dm), fractional_bits));
	}

	bool CodeBuffer::EmitVcvtS32F32Q(unsigned qd, unsigned qm, unsigned fractional_bits)
	{
		if (!IsQRegister(qd) || !IsQRegister(qm) || fractional_bits > 32)
			return false;
		return EmitU32(EncodeVcvtS32F32Q(MapNeonQRegister(qd),
			MapNeonQRegister(qm), fractional_bits));
	}
	...
	bool CodeBuffer::EmitVaddF32D(unsigned dd, unsigned dn, unsigned dm)
	{
		if (!IsDRegister(dd) || !IsDRegister(dn) || !IsDRegister(dm))
			return false;
		return EmitU32(EncodeVaddF32D(MapNeonDRegister(dd),
			MapNeonDRegister(dn), MapNeonDRegister(dm)));
	}

	bool CodeBuffer::EmitVsubF32D(unsigned dd, unsigned dn, unsigned dm)
	{
		if (!IsDRegister(dd) || !IsDRegister(dn) || !IsDRegister(dm))
			return false;
		return EmitU32(EncodeVsubF32D(MapNeonDRegister(dd),
			MapNeonDRegister(dn), MapNeonDRegister(dm)));
	}

	bool CodeBuffer::EmitVmulF32D(unsigned dd, unsigned dn, unsigned dm)
	{
		if (!IsDRegister(dd) || !IsDRegister(dn) || !IsDRegister(dm))
			return false;
		return EmitU32(EncodeVmulF32D(MapNeonDRegister(dd),
			MapNeonDRegister(dn), MapNeonDRegister(dm)));
	}

	bool CodeBuffer::EmitVmlaF32D(unsigned dd, unsigned dn, unsigned dm)
	{
		if (!IsDRegister(dd) || !IsDRegister(dn) || !IsDRegister(dm))
			return false;
		return EmitU32(EncodeVmlaF32D(MapNeonDRegister(dd),
			MapNeonDRegister(dn), MapNeonDRegister(dm)));
	}

	bool CodeBuffer::EmitVmlsF32D(unsigned dd, unsigned dn, unsigned dm)
	{
		if (!IsDRegister(dd) || !IsDRegister(dn) || !IsDRegister(dm))
			return false;
		return EmitU32(EncodeVmlsF32D(MapNeonDRegister(dd),
			MapNeonDRegister(dn), MapNeonDRegister(dm)));
	}

	bool CodeBuffer::EmitVmulF32DByLane(unsigned dd, unsigned dn, unsigned dm, unsigned lane)
	{
		if (!IsDRegister(dd) || !IsDRegister(dn) || !IsDRegister(dm) || lane >= 2)
			return false;
		const unsigned mapped_dm = MapNeonDRegister(dm);
		return mapped_dm < 16 && EmitU32(EncodeVmulF32DByLane(MapNeonDRegister(dd),
			MapNeonDRegister(dn), mapped_dm, lane));
	}

	bool CodeBuffer::EmitVmlaF32DByLane(unsigned dd, unsigned dn, unsigned dm, unsigned lane)
	{
		if (!IsDRegister(dd) || !IsDRegister(dn) || !IsDRegister(dm) || lane >= 2)
			return false;
		const unsigned mapped_dm = MapNeonDRegister(dm);
		return mapped_dm < 16 && EmitU32(EncodeVmlaF32DByLane(MapNeonDRegister(dd),
			MapNeonDRegister(dn), mapped_dm, lane));
	}

	bool CodeBuffer::EmitVmlsF32DByLane(unsigned dd, unsigned dn, unsigned dm, unsigned lane)
	{
		if (!IsDRegister(dd) || !IsDRegister(dn) || !IsDRegister(dm) || lane >= 2)
			return false;
		const unsigned mapped_dm = MapNeonDRegister(dm);
		return mapped_dm < 16 && EmitU32(EncodeVmlsF32DByLane(MapNeonDRegister(dd),
			MapNeonDRegister(dn), mapped_dm, lane));
	}
	...
	bool CodeBuffer::EmitVmlaF32Q(unsigned qd, unsigned qn, unsigned qm)
	{
		if (!IsQRegister(qd) || !IsQRegister(qn) || !IsQRegister(qm))
			return false;
		return EmitU32(EncodeVmlaF32Q(MapNeonQRegister(qd), MapNeonQRegister(qn), MapNeonQRegister(qm)));
	}

	bool CodeBuffer::EmitVmlsF32Q(unsigned qd, unsigned qn, unsigned qm)
	{
		if (!IsQRegister(qd) || !IsQRegister(qn) || !IsQRegister(qm))
			return false;
		return EmitU32(EncodeVmlsF32Q(MapNeonQRegister(qd), MapNeonQRegister(qn), MapNeonQRegister(qm)));
	}

	bool CodeBuffer::EmitVmulF32QByLane(unsigned qd, unsigned qn, unsigned dm, unsigned lane)
	{
		if (!IsQRegister(qd) || !IsQRegister(qn) || !IsDRegister(dm) || lane >= 2)
			return false;
		const unsigned mapped_dm = MapNeonDRegister(dm);
		return mapped_dm < 16 && EmitU32(EncodeVmulF32QByLane(MapNeonQRegister(qd),
			MapNeonQRegister(qn), mapped_dm, lane));
	}

	bool CodeBuffer::EmitVmlaF32QByLane(unsigned qd, unsigned qn, unsigned dm, unsigned lane)
	{
		if (!IsQRegister(qd) || !IsQRegister(qn) || !IsDRegister(dm) || lane >= 2)
			return false;
		const unsigned mapped_dm = MapNeonDRegister(dm);
		return mapped_dm < 16 && EmitU32(EncodeVmlaF32QByLane(MapNeonQRegister(qd),
			MapNeonQRegister(qn), mapped_dm, lane));
	}

	bool CodeBuffer::EmitVmlsF32QByLane(unsigned qd, unsigned qn, unsigned dm, unsigned lane)
	{
		if (!IsQRegister(qd) || !IsQRegister(qn) || !IsDRegister(dm) || lane >= 2)
			return false;
		const unsigned mapped_dm = MapNeonDRegister(dm);
		return mapped_dm < 16 && EmitU32(EncodeVmlsF32QByLane(MapNeonQRegister(qd),
			MapNeonQRegister(qn), mapped_dm, lane));
	}

	bool CodeBuffer::EmitVrecpeF32D(unsigned dd, unsigned dm)
	{
		if (!IsDRegister(dd) || !IsDRegister(dm))
			return false;
		return EmitU32(EncodeVrecpeF32D(MapNeonDRegister(dd), MapNeonDRegister(dm)));
	}

	bool CodeBuffer::EmitVrecpsF32D(unsigned dd, unsigned dn, unsigned dm)
	{
		if (!IsDRegister(dd) || !IsDRegister(dn) || !IsDRegister(dm))
			return false;
		return EmitU32(EncodeVrecpsF32D(MapNeonDRegister(dd),
			MapNeonDRegister(dn), MapNeonDRegister(dm)));
	}

	bool CodeBuffer::EmitVrsqrteF32D(unsigned dd, unsigned dm)
	{
		if (!IsDRegister(dd) || !IsDRegister(dm))
			return false;
		return EmitU32(EncodeVrsqrteF32D(MapNeonDRegister(dd), MapNeonDRegister(dm)));
	}

	bool CodeBuffer::EmitVrsqrtsF32D(unsigned dd, unsigned dn, unsigned dm)
	{
		if (!IsDRegister(dd) || !IsDRegister(dn) || !IsDRegister(dm))
			return false;
		return EmitU32(EncodeVrsqrtsF32D(MapNeonDRegister(dd),
			MapNeonDRegister(dn), MapNeonDRegister(dm)));
	}
	...
	bool CodeBuffer::EmitVsriI32Q(unsigned qd, unsigned qm, u8 amount)
	{
		if (!IsQRegister(qd) || !IsQRegister(qm) || amount == 0 || amount >= 32)
			return false;
		return EmitU32(EncodeVsriI32Q(MapNeonQRegister(qd), MapNeonQRegister(qm), amount));
	}
	...
	bool CodeBuffer::EmitVceqI32D(unsigned dd, unsigned dn, unsigned dm)
	{
		if (!IsDRegister(dd) || !IsDRegister(dn) || !IsDRegister(dm))
			return false;
		return EmitU32(EncodeVceqI32D(MapNeonDRegister(dd),
			MapNeonDRegister(dn), MapNeonDRegister(dm)));
	}
	...
	bool CodeBuffer::EmitVceqI32ZeroD(unsigned dd, unsigned dm)
	{
		if (!IsDRegister(dd) || !IsDRegister(dm))
			return false;
		return EmitU32(EncodeVceqI32ZeroD(MapNeonDRegister(dd), MapNeonDRegister(dm)));
	}

	bool CodeBuffer::EmitVceqI32ZeroQ(unsigned qd, unsigned qm)
	{
		if (!IsQRegister(qd) || !IsQRegister(qm))
			return false;
		return EmitU32(EncodeVceqI32ZeroQ(MapNeonQRegister(qd), MapNeonQRegister(qm)));
	}

	bool CodeBuffer::EmitVcltS32ZeroQ(unsigned qd, unsigned qm)
	{
		if (!IsQRegister(qd) || !IsQRegister(qm))
			return false;
		return EmitU32(EncodeVcltS32ZeroQ(MapNeonQRegister(qd), MapNeonQRegister(qm)));
	}
	...
	bool CodeBuffer::EmitVextI8D(unsigned dd, unsigned dn, unsigned dm, u8 byte_offset)
	{
		if (!IsDRegister(dd) || !IsDRegister(dn) || !IsDRegister(dm) || byte_offset >= 8)
			return false;
		return EmitU32(EncodeVextI8D(MapNeonDRegister(dd), MapNeonDRegister(dn),
			MapNeonDRegister(dm), byte_offset));
	}
	...
	bool CodeBuffer::EmitVandD(unsigned dd, unsigned dn, unsigned dm)
	{
		if (!IsDRegister(dd) || !IsDRegister(dn) || !IsDRegister(dm))
			return false;
		return EmitU32(EncodeVandD(MapNeonDRegister(dd),
			MapNeonDRegister(dn), MapNeonDRegister(dm)));
	}
	...
	bool CodeBuffer::EmitVbitD(unsigned dd, unsigned dn, unsigned dm)
	{
		if (!IsDRegister(dd) || !IsDRegister(dn) || !IsDRegister(dm))
			return false;
		return EmitU32(EncodeVbitD(MapNeonDRegister(dd),
			MapNeonDRegister(dn), MapNeonDRegister(dm)));
	}

	bool CodeBuffer::EmitVbitQ(unsigned qd, unsigned qn, unsigned qm)
	{
		if (!IsQRegister(qd) || !IsQRegister(qn) || !IsQRegister(qm))
			return false;
		return EmitU32(EncodeVbitQ(MapNeonQRegister(qd), MapNeonQRegister(qn), MapNeonQRegister(qm)));
	}
	...
	bool CodeBuffer::ReadInstruction(size_t instruction_offset, u32* instruction) const
	{
		...
		std::memcpy(instruction, ReadBase() + instruction_offset,
			sizeof(*instruction));
		return true;
	}
		...
		Condition condition)
	{
		...
		u32 placeholder = 0;
		std::memcpy(&placeholder, ReadBase() + instruction_offset,
			sizeof(placeholder));
	...
	bool CodeBuffer::EmitStmIa(unsigned rn, u16 register_list, bool writeback)
	{
		// ARM ARM A8.6.189, A1 encoding. W selects the architected post-transfer
		// base update by four bytes per listed register. The base-in-list case is
		// excluded even without writeback so every emitted form has one meaning.
		// PC is excluded from both roles.
		if (!IsGeneralRegister(rn) || register_list == 0 ||
			(register_list & ((1u << rn) | (1u << 15))) != 0)
		{
			return false;
		}
		return EmitU32(CondBits(Condition::AL) | STMIA |
			(static_cast<u32>(writeback) << 21) |
			((rn & 0xfu) << 16) | register_list);
	}

	bool CodeBuffer::EmitLdmIa(unsigned rn, u16 register_list, bool writeback)
	{
		// ARM ARM A8.6.53, A1 encoding. Ascending memory words map to
		// ascending register numbers. Keep the same unambiguous subset as STMIA:
		// no PC and no base-in-list form, even when writeback is disabled.
		if (!IsGeneralRegister(rn) || register_list == 0 ||
			(register_list & ((1u << rn) | (1u << 15))) != 0)
		{
			return false;
		}
		return EmitU32(CondBits(Condition::AL) | LDMIA |
			(static_cast<u32>(writeback) << 21) |
			((rn & 0xfu) << 16) | register_list);
	}
	...
	bool CodeBuffer::PatchMovImm32(size_t instruction_offset, unsigned rd, u32 value)
	{
		...
		if (!EnsureWritable(m_offset))
			return false;
		std::memcpy(m_write_base + instruction_offset, &movw, sizeof(movw));
		std::memcpy(m_write_base + instruction_offset + sizeof(movw), &movt,
			sizeof(movt));
	...
	bool CodeBuffer::Flush()
	{
		...
		if (!m_write_domain_open)
			return false;
		m_write_domain_open = false;
		const bool published = VitaVM::EndJitWriteAndSync(
	...
	bool CodeBuffer::EnsureWritable(size_t required_capacity)
	{
		if (m_write_domain_open)
		{
			if (required_capacity <= m_write_capacity)
				return true;
			void* write_base = m_write_base;
			if (!VitaVM::ExpandJitWrite(m_offset, required_capacity,
					&write_base, &m_write_capacity))
			{
				return false;
			}
			m_write_base = static_cast<u8*>(write_base);
			return true;
		}
		if (!m_base || required_capacity > m_capacity)
			return false;
		void* write_base = nullptr;
		m_write_domain_open = VitaVM::BeginJitWrite(m_base, m_offset, m_capacity,
			&write_base, &m_write_capacity);
		m_write_base = m_write_domain_open ?
			static_cast<u8*>(write_base) : nullptr;
	...
	bool CodeBuffer::CloseWriteDomain()
	{
		if (!m_write_domain_open)
			return true;
		m_write_domain_open = false;
		const bool ended = VitaVM::EndJitWrite();
		m_write_base = nullptr;
		m_write_capacity = 0;
		return ended;
	}
		...
		Condition condition)
	{
		pxAssert(IsRegister(rd));
		pxAssert(IsRegister(rn));
		return CondBits(condition) | DATA_PROCESSING_IMM | OPCODE_ADD | (set_flags ? SET_FLAGS : 0) |
	...
	u32 EncodeBfi(unsigned rd, unsigned rn, u8 lsb, u8 width)
	{
		...
		const u32 msb = static_cast<u32>(lsb) + static_cast<u32>(width) - 1u;
		return CondBits(Condition::AL) | BFI | ((msb & 0x1fu) << 16) |
	...
	u32 EncodeStrImm12(unsigned rd, unsigned rn, u16 offset, Condition condition)
	{
		pxAssert(IsLowRegister(rd));
		pxAssert(IsLowRegister(rn));
		pxAssert(offset <= 0x0fff);
		return CondBits(condition) | STR_IMM | ((rn & 0xfu) << 16) |
			((rd & 0xfu) << 12) | offset;
	}
	...
	u32 EncodeVdupI32DFromCore(unsigned dd, unsigned rt)
	{
		...
		return VDUP_I32_D_CORE | NeonDn(dd) | ((rt & 0xfu) << 12);
	}
	...
	u32 EncodeVdupI32DFromQlane(unsigned dd, unsigned qm, u8 lane)
	{
		...
		pxAssert(lane <= 3);
		const unsigned dm = qm * 2u + (lane >> 1);
		return VDUP_I32_D_LANE | NeonDd(dd) | NeonDm(dm) |
			((static_cast<u32>(lane) & 1u) << 19);
	}
	...
	u32 EncodeVdupI32QFromQlane(unsigned qd, unsigned qm, u8 lane)
	{
		...
		pxAssert(lane <= 3);
		const unsigned dm = qm * 2u + (lane >> 1);
		return VDUP_I32_Q_LANE | NeonQd(qd) | NeonDm(dm) |
			((static_cast<u32>(lane) & 1u) << 19);
	}
	...
	{
		pxAssert(IsQRegister(qd));
		pxAssert(left_shift <= 24 && (left_shift & 7u) == 0);
		// A8.6.336: i:imm3:imm4 carries imm8, while cmode 000x/001x/
		// 010x/011x selects LSL #0/#8/#16/#24 for 32-bit elements.
		return VMOV_I32_Q_IMM |
			((static_cast<u32>(imm8) & 0x80u) << 17) |
			((static_cast<u32>(imm8) & 0x70u) << 12) |
			((static_cast<u32>(left_shift / 8u) * 2u) << 8) |
			NeonQd(qd) | (static_cast<u32>(imm8) & 0x0fu);
	}
	...
	u32 EncodeVbicI32Q(unsigned qd, u8 imm8, u8 left_shift)
	{
		pxAssert(IsQRegister(qd));
		pxAssert(left_shift <= 24 && (left_shift & 7u) == 0);
		// A8.6.277 uses the same i:imm3:imm4 byte and byte-position
		// fields as VMOV.I32, with the VBIC cmode low bit set.
		return VBIC_I32_Q_IMM |
			((static_cast<u32>(imm8) & 0x80u) << 17) |
			((static_cast<u32>(imm8) & 0x70u) << 12) |
			((static_cast<u32>(left_shift / 8u) * 2u + 1u) << 8) |
			NeonQd(qd) | (static_cast<u32>(imm8) & 0x0fu);
	}
	...
	u32 EncodeLdrsbImm8PostIndex(unsigned rd, unsigned rn, u8 offset)
	{
		...
		pxAssert(IsLowRegister(rn));
		pxAssert(rd != rn);
		return CondBits(Condition::AL) | LDRSB_IMM_POST_INDEX |
			((rn & 0xfu) << 16) | ((rd & 0xfu) << 12) |
			((static_cast<u32>(offset) & 0xf0u) << 4) | (offset & 0x0fu);
	}
	...
	u32 EncodeVcvtF32S32Q(unsigned qd, unsigned qm, unsigned fractional_bits)
	{
		pxAssert(IsQRegister(qd));
		pxAssert(IsQRegister(qm));
		pxAssert(fractional_bits <= 32);
		const u32 opcode = fractional_bits == 0 ? VCVT_F32_S32_Q :
			(VCVT_F32_S32_FIXED_Q | ((64u - fractional_bits) << 16));
		return opcode | NeonQd(qd) | NeonQm(qm);
	}

	u32 EncodeVcvtF32S32D(unsigned dd, unsigned dm, unsigned fractional_bits)
	{
		pxAssert(IsDRegister(dd));
		pxAssert(IsDRegister(dm));
		pxAssert(fractional_bits <= 32);
		const u32 opcode = fractional_bits == 0 ? VCVT_F32_S32_D :
			(VCVT_F32_S32_FIXED_D | ((64u - fractional_bits) << 16));
		return opcode | NeonDd(dd) | NeonDm(dm);
	}

	u32 EncodeVcvtS32F32D(unsigned dd, unsigned dm, unsigned fractional_bits)
	{
		pxAssert(IsDRegister(dd));
		pxAssert(IsDRegister(dm));
		pxAssert(fractional_bits <= 32);
		const u32 opcode = fractional_bits == 0 ? VCVT_S32_F32_D :
			(VCVT_S32_F32_FIXED_D | ((64u - fractional_bits) << 16));
		return opcode | NeonDd(dd) | NeonDm(dm);
	}

	u32 EncodeVcvtS32F32Q(unsigned qd, unsigned qm, unsigned fractional_bits)
	{
		pxAssert(IsQRegister(qd));
		pxAssert(IsQRegister(qm));
		pxAssert(fractional_bits <= 32);
		const u32 opcode = fractional_bits == 0 ? VCVT_S32_F32_Q :
			(VCVT_S32_F32_FIXED_Q | ((64u - fractional_bits) << 16));
		return opcode | NeonQd(qd) | NeonQm(qm);
	}
	...
	u32 EncodeVaddF32D(unsigned dd, unsigned dn, unsigned dm)
	{
		pxAssert(IsDRegister(dd));
		pxAssert(IsDRegister(dn));
		pxAssert(IsDRegister(dm));
		return VADD_F32_D | NeonDd(dd) | NeonDn(dn) | NeonDm(dm);
	}

	u32 EncodeVsubF32D(unsigned dd, unsigned dn, unsigned dm)
	{
		pxAssert(IsDRegister(dd));
		pxAssert(IsDRegister(dn));
		pxAssert(IsDRegister(dm));
		return VSUB_F32_D | NeonDd(dd) | NeonDn(dn) | NeonDm(dm);
	}

	u32 EncodeVmulF32D(unsigned dd, unsigned dn, unsigned dm)
	{
		pxAssert(IsDRegister(dd));
		pxAssert(IsDRegister(dn));
		pxAssert(IsDRegister(dm));
		return VMUL_F32_D | NeonDd(dd) | NeonDn(dn) | NeonDm(dm);
	}

	u32 EncodeVmlaF32D(unsigned dd, unsigned dn, unsigned dm)
	{
		pxAssert(IsDRegister(dd));
		pxAssert(IsDRegister(dn));
		pxAssert(IsDRegister(dm));
		return VMLA_F32_D | NeonDd(dd) | NeonDn(dn) | NeonDm(dm);
	}

	u32 EncodeVmlsF32D(unsigned dd, unsigned dn, unsigned dm)
	{
		pxAssert(IsDRegister(dd));
		pxAssert(IsDRegister(dn));
		pxAssert(IsDRegister(dm));
		return VMLS_F32_D | NeonDd(dd) | NeonDn(dn) | NeonDm(dm);
	}

	u32 EncodeVmulF32DByLane(unsigned dd, unsigned dn, unsigned dm, unsigned lane)
	{
		pxAssert(IsDRegister(dd));
		pxAssert(IsDRegister(dn));
		pxAssert(dm < 16);
		pxAssert(lane < 2);
		return VMUL_F32_D_LANE | NeonDd(dd) | NeonDn(dn) | dm | (lane << 5);
	}

	u32 EncodeVmlaF32DByLane(unsigned dd, unsigned dn, unsigned dm, unsigned lane)
	{
		pxAssert(IsDRegister(dd));
		pxAssert(IsDRegister(dn));
		pxAssert(dm < 16);
		pxAssert(lane < 2);
		return VMLA_F32_D_LANE | NeonDd(dd) | NeonDn(dn) | dm | (lane << 5);
	}

	u32 EncodeVmlsF32DByLane(unsigned dd, unsigned dn, unsigned dm, unsigned lane)
	{
		pxAssert(IsDRegister(dd));
		pxAssert(IsDRegister(dn));
		pxAssert(dm < 16);
		pxAssert(lane < 2);
		return VMLS_F32_D_LANE | NeonDd(dd) | NeonDn(dn) | dm | (lane << 5);
	}
	...
	u32 EncodeVmlaF32Q(unsigned qd, unsigned qn, unsigned qm)
	{
		pxAssert(IsQRegister(qd));
		pxAssert(IsQRegister(qn));
		pxAssert(IsQRegister(qm));
		return VMLA_F32_Q | NeonQd(qd) | NeonQn(qn) | NeonQm(qm);
	}

	u32 EncodeVmlsF32Q(unsigned qd, unsigned qn, unsigned qm)
	{
		pxAssert(IsQRegister(qd));
		pxAssert(IsQRegister(qn));
		pxAssert(IsQRegister(qm));
		return VMLS_F32_Q | NeonQd(qd) | NeonQn(qn) | NeonQm(qm);
	}

	u32 EncodeVmulF32QByLane(unsigned qd, unsigned qn, unsigned dm, unsigned lane)
	{
		pxAssert(IsQRegister(qd));
		pxAssert(IsQRegister(qn));
		pxAssert(dm < 16);
		pxAssert(lane < 2);
		return VMUL_F32_Q_LANE | NeonQd(qd) | NeonQn(qn) | dm | (lane << 5);
	}

	u32 EncodeVmlaF32QByLane(unsigned qd, unsigned qn, unsigned dm, unsigned lane)
	{
		pxAssert(IsQRegister(qd));
		pxAssert(IsQRegister(qn));
		pxAssert(dm < 16);
		pxAssert(lane < 2);
		return VMLA_F32_Q_LANE | NeonQd(qd) | NeonQn(qn) | dm | (lane << 5);
	}

	u32 EncodeVmlsF32QByLane(unsigned qd, unsigned qn, unsigned dm, unsigned lane)
	{
		pxAssert(IsQRegister(qd));
		pxAssert(IsQRegister(qn));
		pxAssert(dm < 16);
		pxAssert(lane < 2);
		return VMLS_F32_Q_LANE | NeonQd(qd) | NeonQn(qn) | dm | (lane << 5);
	}

	u32 EncodeVrecpeF32D(unsigned dd, unsigned dm)
	{
		pxAssert(IsDRegister(dd));
		pxAssert(IsDRegister(dm));
		return VRECPE_F32_D | NeonDd(dd) | NeonDm(dm);
	}

	u32 EncodeVrecpsF32D(unsigned dd, unsigned dn, unsigned dm)
	{
		pxAssert(IsDRegister(dd));
		pxAssert(IsDRegister(dn));
		pxAssert(IsDRegister(dm));
		return VRECPS_F32_D | NeonDd(dd) | NeonDn(dn) | NeonDm(dm);
	}

	u32 EncodeVrsqrteF32D(unsigned dd, unsigned dm)
	{
		pxAssert(IsDRegister(dd));
		pxAssert(IsDRegister(dm));
		return VRSQRTE_F32_D | NeonDd(dd) | NeonDm(dm);
	}

	u32 EncodeVrsqrtsF32D(unsigned dd, unsigned dn, unsigned dm)
	{
		pxAssert(IsDRegister(dd));
		pxAssert(IsDRegister(dn));
		pxAssert(IsDRegister(dm));
		return VRSQRTS_F32_D | NeonDd(dd) | NeonDn(dn) | NeonDm(dm);
	}
	...
	u32 EncodeVsriI32Q(unsigned qd, unsigned qm, u8 amount)
	{
		...
		pxAssert(amount > 0 && amount < 32);
		const u8 imm = static_cast<u8>(64 - amount);
		return VSRI_I32_Q | ((static_cast<u32>(imm) & 0x3fu) << 16) |
			   NeonQd(qd) | NeonQm(qm);
	}
	...
	u32 EncodeVceqI32D(unsigned dd, unsigned dn, unsigned dm)
	{
		...
		pxAssert(IsDRegister(dn));
		pxAssert(IsDRegister(dm));
		return VCEQ_I32_D | NeonDd(dd) | NeonDn(dn) | NeonDm(dm);
	}
	...
	u32 EncodeVceqI32ZeroD(unsigned dd, unsigned dm)
	{
		pxAssert(IsDRegister(dd));
		pxAssert(IsDRegister(dm));
		return VCEQ_I32_ZERO_D | NeonDd(dd) | NeonDm(dm);
	}
	...
	u32 EncodeVceqI32ZeroQ(unsigned qd, unsigned qm)
	{
		pxAssert(IsQRegister(qd));
		pxAssert(IsQRegister(qm));
		return VCEQ_I32_ZERO_Q | NeonQd(qd) | NeonQm(qm);
	}
	...
	u32 EncodeVcltS32ZeroQ(unsigned qd, unsigned qm)
	{
		pxAssert(IsQRegister(qd));
		pxAssert(IsQRegister(qm));
		return VCLT_S32_ZERO_Q | NeonQd(qd) | NeonQm(qm);
	}
	...
	u32 EncodeVextI8D(unsigned dd, unsigned dn, unsigned dm, u8 byte_offset)
	{
		pxAssert(IsDRegister(dd));
		pxAssert(IsDRegister(dn));
		pxAssert(IsDRegister(dm));
		pxAssert(byte_offset < 8);
		return VEXT_I8_D | NeonDd(dd) | NeonDn(dn) | NeonDm(dm) |
			   ((static_cast<u32>(byte_offset) & 0x07u) << 8);
	}
	...
	u32 EncodeVandD(unsigned dd, unsigned dn, unsigned dm)
	{
		...
		pxAssert(IsDRegister(dn));
		pxAssert(IsDRegister(dm));
		return VAND_D | NeonDd(dd) | NeonDn(dn) | NeonDm(dm);
	}
	...
	u32 EncodeVbitD(unsigned dd, unsigned dn, unsigned dm)
	{
		...
		pxAssert(IsDRegister(dn));
		pxAssert(IsDRegister(dm));
		return VBIT_D | NeonDd(dd) | NeonDn(dn) | NeonDm(dm);
	}
	...
	u32 EncodeVbitQ(unsigned qd, unsigned qn, unsigned qm)
	{
		...
		pxAssert(IsQRegister(qn));
		pxAssert(IsQRegister(qm));
		return VBIT_Q | NeonQd(qd) | NeonQn(qn) | NeonQm(qm);
	}
