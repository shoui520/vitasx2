...
namespace
{
	...
	std::span<const u8> GetEntrySpan(const ArchiveEntryList& entries, u32 index)
	{
	...
	const ArchiveEntryList& entries, Error* error)
{
	...
	// Complete structural preflight happens before any live machine state is
	// touched. Unlike desktop PCSX2, the Vita core has no VMManager::Reset()
	// implementation available to roll back a partially loaded state.
	...
	if (!size_is(0, SaveStateRaw::PORTABLE_VERSION_ENTRY_PAYLOAD.size()) ||
		!std::equal(SaveStateRaw::PORTABLE_VERSION_ENTRY_PAYLOAD.begin(),
			SaveStateRaw::PORTABLE_VERSION_ENTRY_PAYLOAD.end(), GetEntrySpan(entries, 0).begin()) ||
	...
	if (!ValidatePortableBiosHeader(GetEntrySpan(entries, 1), error))
		return PortableStateLoadResult::RejectedBeforeMutation;
	...
	PreLoadPrep();
	const std::span<const u8> internal = GetEntrySpan(entries, 1);
	...
	{
		SetDefaultError(error, "Portable replay internal state is corrupt or under-consumed.");
		return PortableStateLoadResult::FailedAfterMutation;
	}

	std::memcpy(eeMem->Main, GetEntrySpan(entries, 2).data(), Ps2MemSize::ExposedRam);
	std::memcpy(iopMem->Main, GetEntrySpan(entries, 3).data(), Ps2MemSize::ExposedIopRam);
	std::memcpy(eeHw, GetEntrySpan(entries, 4).data(), sizeof(eeHw));
	std::memcpy(iopHw, GetEntrySpan(entries, 5).data(), sizeof(iopHw));
	std::memcpy(eeMem->Scratch, GetEntrySpan(entries, 6).data(), sizeof(eeMem->Scratch));
	std::memcpy(VU0.Mem, GetEntrySpan(entries, 7).data(), VU0_MEMSIZE);
	std::memcpy(VU1.Mem, GetEntrySpan(entries, 8).data(), VU1_MEMSIZE);
	std::memcpy(VU0.Micro, GetEntrySpan(entries, 9).data(), VU0_PROGSIZE);
	std::memcpy(VU1.Micro, GetEntrySpan(entries, 10).data(), VU1_PROGSIZE);
	...
	if (!LoadStateWrapperEntry(GetEntrySpan(entries, 11), &SPU2::DoPortableState) ||
		!LoadStateWrapperEntry(GetEntrySpan(entries, 12), &USB::DoState) ||
		!LoadStateWrapperEntry(GetEntrySpan(entries, 13), &Pad::Freeze) ||
		!LoadLegacyComponent(GetEntrySpan(entries, 14), GS_COMPONENT))
	{
