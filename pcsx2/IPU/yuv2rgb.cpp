...
u32 g_qemuIpuYuv2RgbNeonBlocks = 0;
#endif
...
#if defined(ARCH_ARM64) && defined(_MSC_VER) && !defined(__clang__)
...
{
	const uint8x8x2_t zipped = vzip_u8(even, odd);
...
__ri void yuv2rgb_neon()
{
	...
	for (int n = 0; n < 8; ++n)
	{
		...
		for (int m = 0; m < 2; ++m)
		{
			...
			const uint8x16_t g = InterleaveEvenOdd(vqmovun_s16(g_even), vqmovun_s16(g_odd));
			const uint8x16_t b = InterleaveEvenOdd(vqmovun_s16(b_even), vqmovun_s16(b_odd));
			const uint8x16x4_t rgba = {r, g, b, alpha};
			vst4q_u8(reinterpret_cast<u8*>(&decoder.rgb32.c[n * 2 + m][0]), rgba);
		}
	}

#if defined(VITASX2_QEMU_VALIDATION)
	++::g_qemuIpuYuv2RgbNeonBlocks;
#endif
}
#endif

#if defined(VITASX2_QEMU_VALIDATION)
void IpuYuv2RgbReferenceForValidation()
{
	yuv2rgb_reference();
}

void IpuYuv2RgbSelectedForValidation()
{
	yuv2rgb();
}
#endif
