...
public:
	...
	__forceinline GSVector4 round() const
	{
		if constexpr (mode == Round_NegInf)
			...
			return GSVector4(std::nearbyint(F32[0]), std::nearbyint(F32[1]), std::nearbyint(F32[2]), std::nearbyint(F32[3]));
		else
			return GSVector4(std::trunc(F32[0]), std::trunc(F32[1]), std::trunc(F32[2]), std::trunc(F32[3]));
	}

	__forceinline GSVector4 floor() const
	{
		return GSVector4(std::floor(F32[0]), std::floor(F32[1]), std::floor(F32[2]), std::floor(F32[3]));
	}

	__forceinline GSVector4 ceil() const
	{
		return GSVector4(std::ceil(F32[0]), std::ceil(F32[1]), std::ceil(F32[2]), std::ceil(F32[3]));
	}
...
#define LOG_POLY5(x, c0, c1, c2, c3, c4, c5) (LOG_POLY4(x, c1, c2, c3, c4, c5).madd(x, GSVector4(c0)))
	...
	__forceinline friend GSVector4 operator/(const GSVector4& v1, const GSVector4& v2)
	{
		return GSVector4(v1.F32[0] / v2.F32[0], v1.F32[1] / v2.F32[1], v1.F32[2] / v2.F32[2], v1.F32[3] / v2.F32[3]);
	}
	...
	{
		return f64(F64[0] * v.F64[0], F64[1] * v.F64[1]);
	}
